on open location theURL
	set theChoice to button returned of (display dialog "Open with which browser?" & return & return & theURL buttons {"Safari (S)", "Edge (E)", "Chrome (C)"} default button "Chrome (C)" with title "Browser Chooser" with icon note giving up after 30)
	...
	else if theChoice is "Safari (S)" then
		do shell script "open -a '/Applications/Safari.app' " & quoted form of theURL
	end if
end open location
